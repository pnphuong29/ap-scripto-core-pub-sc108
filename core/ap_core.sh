...
# Common editors
export AP_EDITOR_ARR=("vi")
# export AP_EDITOR_ARR=("vi" "code" "zed")

source "${HOME}/scripto/core/ap_codes.sh"
source "${HOME}/scripto/core/ap_colors_and_formats.sh"
source "${HOME}/scripto/core/ap_symbols.sh"
source "${HOME}/scripto/core/ap_dir_struct.sh"
...
ap_func_add_path_var() {
    ...
    if [ "${ap_opt_append}" == 0 ]; then
        ap_cmd="export ${ap_ref_path}=\"\"${ap_path}\":${!ap_ref_path}\""
        # eval "$(printf "%s" "export ${ap_ref_path}=\"${ap_path}\":${!ap_ref_path}")"
    else
        ap_cmd="export ${ap_ref_path}=\"${!ap_ref_path}:\"${ap_path}\"\""
        # eval "$(printf "%s" "export ${ap_ref_path}=${!ap_ref_path}:\"${ap_path}\"")"
    fi
    # aplogdebug "Execute [${ap_cmd}\n]"
...
# }
ap_func_rm_path_var() {
    local ap_search_path="$1"

    if [ -z "${ap_search_path:-}" ]; then
        apshowmsgerror "Missing argument <PATH_VAR>"
        aprtn_err_missing_argument
    fi
    ...
    ap_new_path="$(echo "${PATH}" | gsed "s@:${ap_search_path}@@g" | gsed "s@${ap_search_path}:@@g")"
    echo "${ap_new_path}"
    export PATH="${ap_new_path}"
    aprtn_success
}

# Function to log debug messages to bash completion log file
_ap_debug_completion() {
    echo "$@" >>"${AP_LOGS_BASH_COMPLETION_FILE}"
}
