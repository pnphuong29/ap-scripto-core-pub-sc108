...
ap_func_init_xxx() {
    ...
    export AP_URL_XXX_DL="https://jdk.java.net/archive/"

    alias zxxxconfig="cd \${AP_XXX_CONF_DIR}"
...
ap_func_create_dirstruct_xxx() {
    ...
    xxx completion bash >"${AP_COMPLETIONS_DIR}/ap_completion_xxx.bash"

    # Find and append " xxxalias" at the end of lines containing "__start_xxx xxx"
    if [ -f "${AP_COMPLETIONS_DIR}/ap_completion_xxx.bash" ]; then
        gsed -i.bak 's/\(__start_xxx xxx\)$/\1 xxxalias/g' "${AP_COMPLETIONS_DIR}/ap_completion_xxx.bash"
        rm -f "${AP_COMPLETIONS_DIR}/ap_completion_xxx.bash.bak"
    fi
...
ap_func_setup_xxx() {
    ...
    if [ "${AP_OS_TYPE}" == "${AP_OS_TYPE_MACOS}" ]; then
        ...
        if [[ "$(uname -m)" == 'arm64' ]]; then
            ap_dl_link="$(curl --silent "https://api.github.com/repos/owner/xxx/releases" | jq -r '.[0].assets[].browser_download_url' | grep "macos" | grep arm | grep -v sha256)"
        elif [[ "$(uname -m)" == 'x86_64' ]]; then
            ap_dl_link="$(curl --silent "https://api.github.com/repos/owner/xxx/releases" | jq -r '.[0].assets[].browser_download_url' | grep "macos" | grep x86_64 | grep -v sha256)"
        fi
        apshowmsginfo "Downloading [xxx] for macos from ${ap_dl_link}\n"
        curl -SL "${ap_dl_link}" >xxx.tar.gz
        ...
        ap_dl_link="$(curl --silent "https://api.github.com/repos/owner/xxx/releases" | jq -r '.[0].assets[].browser_download_url' | grep "linux" | grep x86_64 | grep -v sha256 | grep musl)"
        apshowmsginfo "Downloading [xxx] for linux from ${ap_dl_link}\n"
        curl -SL "${ap_dl_link}" >xxx.tar.gz
        sudo dpkg -i xxx.deb
    ...
    curl -SL "$(ap_func_crawl_web_url "${AP_URL_XXX_DL}" | grep "${ap_xxx_setup_version}" | grep "${ap_os}" | grep x64 | grep -v sha)" >"xxx.tar.gz"
