...
ap_func_create_dirstruct_uv() {
    apshowmsginfo "Generate [uv] bash autocomplete\n"
...
ap_func_rm_dirstruct_uv() {
    apshowmsginfo "Remove [${AP_COMPLETIONS_DIR}/ap_completion_uv.bash]\n"
...
ap_func_setup_uv() {
    ...
    uv python install cpython-3.7.9
    uv python install cpython-3.8.20
    uv python install cpython-3.9.21
    uv python install cpython-3.10.16
    uv python install cpython-3.11.11
