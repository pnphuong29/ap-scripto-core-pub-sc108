...
ap_func_create_global_symlink_certbot() {
    if [ -f "/usr/bin/certbot" ]; then
        apshowmsginfo "Create symlink from [/snap/bin/certbot] to [/usr/bin/certbot]\n"
...
ap_func_rm_global_symlink_certbot() {
    if [ -f "/usr/bin/certbot" ]; then
        apshowmsginfo "Remove [/usr/bin/certbot]\n"
