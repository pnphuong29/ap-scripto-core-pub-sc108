...
ap_func_create_dirstruct_ripgrep() {
    apshowmsginfo "Create symlink from [${AP_SOFT_DIR}/bin/rg] to [${AP_SOFT_DIR}/ripgrep/rg]\n"
    ln -sf "${AP_SOFT_DIR}/ripgrep/rg" "${AP_SOFT_DIR}/bin/rg"

    apshowmsginfo "Create symlink from [${AP_COMPLETIONS_DIR}/ap_completion_rg.bash] to [${AP_SOFT_DIR}/ripgrep/complete/rg.bash]\n"
    ln -sf "${AP_SOFT_DIR}/ripgrep/complete/rg.bash" "${AP_COMPLETIONS_DIR}/ap_completion_rg.bash"

    apshowmsginfo "Create symlink from [${AP_MAN_DIR}/man1/rg.1] to [${AP_SOFT_DIR}/ripgrep/doc/rg.1]\n"
...
ap_func_rm_dirstruct_ripgrep() {
    apshowmsginfo "Remove [${AP_SOFT_DIR}/bin/rg]\n"
    rm -f "${AP_SOFT_DIR}/bin/rg"

    apshowmsginfo "Remove [${AP_COMPLETIONS_DIR}/ap_completion_rg.bash]\n"
    rm -f "${AP_COMPLETIONS_DIR}/ap_completion_rg.bash"

    apshowmsginfo "Remove [${AP_MAN_DIR}/man1/rg.1]\n"
...
ap_func_create_global_symlink_ripgrep() {
    if [ -f "${AP_SOFT_DIR}/bin/rg" ]; then
        apshowmsginfo "Create symlink from [/usr/local/bin/rg] to [${AP_SOFT_DIR}/bin/rg]\n"
...
ap_func_rm_global_symlink_ripgrep() {
    if [ -f "/usr/local/bin/rg" ]; then
        apshowmsginfo "Remove [/usr/local/bin/rg]\n"
