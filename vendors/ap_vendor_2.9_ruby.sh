...
ap_func_create_dirstruct_ruby() {
    # Open below comments if you need to generate completions for [ruby, gem, rails, rake, bundle, jruby]
    # apshowmsginfo "Generate [ruby, gem, rails, rake, bundle] bash autocomplete\n"
    # mkdir -p "${AP_SOFT_DIR}/ruby"
    # cd "${AP_SOFT_DIR}/ruby"
    # git clone "https://github.com/mernen/completion-ruby"

    # apshowmsginfo "Create symlinks for [ruby, gem, rails, rake, bundle, jruby] completions\n"
...
ap_func_rm_dirstruct_ruby() {
    apshowmsginfo "Remove directories [${AP_SOFT_DIR}/ruby]\n"
    rm -rf "${AP_SOFT_DIR}/ruby"

    apshowmsginfo "Remove bash completions for [ruby, gem, rails, rake, bundle, jruby]\n"
