...
ap_func_create_dirstruct_rbenv() {
    # apshowmsginfo "Set rbenv local version [${AP_RUBY_VERSION_DEFAULT}] at [${HOME}]\n"
    # cd "${HOME}"
...
ap_func_rm_dirstruct_rbenv() {
    apshowmsginfo "Remove [${AP_SOFT_DIR}/bin/rbenv]\n"
    rm -f "${AP_SOFT_DIR}/bin/rbenv"

    apshowmsginfo "Remove [${AP_COMPLETIONS_DIR}/ap_completion_rbenv.bash]\n"
    rm -f "${AP_COMPLETIONS_DIR}/ap_completion_rbenv.bash"

    apshowmsginfo "Remove [${AP_MAN_DIR}/man1/rbenv.1]\n"
...
ap_func_create_global_symlink_rbenv() {
    if [ -f "${HOME}/.rbenv/shims/ruby" ]; then
        apshowmsginfo "Create symlink from [/usr/local/bin/ruby] to [${HOME}/.rbenv/shims/ruby]\n"
        sudo ln -sf "${HOME}/.rbenv/shims/ruby" "/usr/local/bin/ruby"
    fi

    if [ -f "${HOME}/.rbenv/shims/gem" ]; then
        apshowmsginfo "Create symlink from [/usr/local/bin/gem] to [${HOME}/.rbenv/shims/gem]\n"
        sudo ln -sf "${HOME}/.rbenv/shims/gem" "/usr/local/bin/gem"
    fi

    if [ -f "${HOME}/.rbenv/shims/bundle" ]; then
        apshowmsginfo "Create symlink from [/usr/local/bin/bundle] to [${HOME}/.rbenv/shims/bundle]\n"
        sudo ln -sf "${HOME}/.rbenv/shims/bundle" "/usr/local/bin/bundle"
    fi

    if [ -f "${HOME}/.rbenv/shims/bundler" ]; then
        apshowmsginfo "Create symlink from [/usr/local/bin/bundler] to [${HOME}/.rbenv/shims/bundler]\n"
...
ap_func_rm_global_symlink_rbenv() {
    apshowmsginfo "Remove [/usr/local/bin/ruby]\n"
    sudo rm -f "/usr/local/bin/ruby"

    apshowmsginfo "Remove [/usr/local/bin/gem]\n"
    sudo rm -f "/usr/local/bin/gem"

    apshowmsginfo "Remove [/usr/local/bin/bundle]\n"
    sudo rm -f "/usr/local/bin/bundle"

    apshowmsginfo "Remove [/usr/local/bin/bundler]\n"
...
ap_func_setup_rbenv() {
    ...
    # Install ruby
    apshowmsginfo "Install [ruby v${AP_RUBY_VERSION_DEFAULT}]\n"
    rbenv install "${AP_RUBY_VERSION_DEFAULT}"

    # Set default global ruby version
    apshowmsginfo "Set rbenv global version [${AP_RUBY_VERSION_DEFAULT}]\n"
    rbenv global "${AP_RUBY_VERSION_DEFAULT}"

    # Install gems
    apshowmsginfo "Installing [neovim, solargraph, asciidoctor]\n"
    gem install \
        neovim \
        solargraph \
        asciidoctor
