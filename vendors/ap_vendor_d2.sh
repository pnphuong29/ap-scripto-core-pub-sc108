...
alias apcreateglobalsymlinkd2="ap_func_create_global_symlink_d2"
ap_func_create_global_symlink_d2() {
    if [[ "${AP_OS_TYPE}" == "${AP_OS_TYPE_MACOS}" ]]; then
        if [ -f "${HOMEBREW_PREFIX}/bin/d2" ]; then
            apshowmsginfo "Create symlink from [/usr/local/bin/d2] to [${HOMEBREW_PREFIX}/bin/d2]\n"
            sudo ln -sf "${HOMEBREW_PREFIX}/bin/d2" "/usr/local/bin/d2"
        fi
    elif [[ "${AP_OS_TYPE}" == "${AP_OS_TYPE_UBUNTU}" ]]; then
        if [ -f "${HOME}/go/bin/d2" ]; then
            apshowmsginfo "Create symlink from [/usr/local/bin/d2] to [${HOME}/go/bin/d2]\n"
            sudo ln -sf "${HOME}/go/bin/d2" "/usr/local/bin/d2"
        fi
    fi
}

alias aprmglobalsymlinkd2="ap_func_rm_global_symlink_d2"
ap_func_rm_global_symlink_d2() {
    if [ -f "/usr/local/bin/d2" ]; then
        apshowmsginfo "Remove [/usr/local/bin/d2]\n"
        sudo rm -f "/usr/local/bin/d2"
    fi
}
