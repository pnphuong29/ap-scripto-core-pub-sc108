...
ap_func_setup_shellcheck() {
    ...
    curl -SL \
        "$(curl --silent https://api.github.com/repos/koalaman/shellcheck/releases | jq -r '.[0].assets[].browser_download_url' | grep "${ap_os}" | grep x86_64 | grep -v sha256)" >shellcheck.tar.xz
