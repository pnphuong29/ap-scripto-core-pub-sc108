...
ap_func_create_dir_struct_fd() {
    apshowmsginfo "Create symlink from [${AP_SOFT_DIR}/bin/fd] to [${AP_SOFT_DIR}/fd/fd]\n"
    ln -sf "${AP_SOFT_DIR}/fd/fd" "${AP_SOFT_DIR}/bin/fd"

    apshowmsginfo "Create symlink from [${AP_COMPLETIONS_DIR}/ap_completion_fd.bash] to [${AP_SOFT_DIR}/fd/complete/fd.bash]\n"
    ln -sf "${AP_SOFT_DIR}/fd/autocomplete/fd.bash" "${AP_COMPLETIONS_DIR}/ap_completion_fd.bash"

    apshowmsginfo "Create symlink from [${AP_MAN_DIR}/man1/fd.1] to [${AP_SOFT_DIR}/fd/fd.1]\n"
    ln -sf "${AP_SOFT_DIR}/fd/fd.1" "${AP_MAN_DIR}/man1/fd.1"

    if [ ! -d "${HOME}/.config/fd" ]; then
        apshowmsginfo "Create directory [${HOME}/.config/fd]"
...
ap_func_rm_dirstruct_fd() {
    apshowmsginfo "Remove [${AP_SOFT_DIR}/bin/fd]\n"
    rm -f "${AP_SOFT_DIR}/bin/fd"

    apshowmsginfo "Remove [${AP_COMPLETIONS_DIR}/ap_completion_fd.bash]\n"
    rm -f "${AP_COMPLETIONS_DIR}/ap_completion_fd.bash"

    apshowmsginfo "Remove [${AP_MAN_DIR}/man1/fd.1]\n"
...
ap_func_create_global_symlink_fd() {
    if [ -f "${AP_SOFT_DIR}/bin/fd" ]; then
        apshowmsginfo "Create symlink from [/usr/local/bin/fd] to [${AP_SOFT_DIR}/bin/fd]\n"
...
ap_func_rm_global_symlink_fd() {
    if [ -f "/usr/local/bin/fd" ]; then
        apshowmsginfo "Remove [/usr/local/bin/fd]\n"
